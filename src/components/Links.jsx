...
const Links = () => {
  ...
  // MAIN ICON HANDLER → supports both Lucide + Iconify
  const getIconComponent = (iconName) => {
    ...
    if (iconName.startsWith("Lucide:")) {
      ...
      return IconComponent ? <IconComponent size={22} /> : <LucideIcons.Link size={20} />
    }

    // Default → Iconify
    return <Icon icon={iconName} width="22" height="22" />
  }
  ...
  return (
    <div ref={linksRef} className="space-y-4 mb-8">
      {links.map((link) => (
        ...
        >
          {/* Background Glow */}
          <div className={`absolute inset-0 bg-gradient-to-r ${link.color} rounded-2xl opacity-0 group-hover:opacity-20 blur transition-opacity duration-300`} />

          {/* Card */}
          <div className="relative glass-dark rounded-2xl p-4 border border-white/10 group-hover:border-white/30 transition-all duration-300 group-hover:scale-105">
